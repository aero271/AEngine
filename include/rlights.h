...
#       define GL_HEADER "glad_gles2.h"    // Required for: OpenGL functionality
#   else
...
#   endif
    ...
    "{"
        ...
        "vec3 T = normalize(vec3(" RLG_SHADER_UNIFORM_MATRIX_MODEL "*vec4(" RLG_SHADER_ATTRIB_TANGENT ".xyz, 0.0)));"
        "vec3 B = normalize(cross(N, T)*" RLG_SHADER_ATTRIB_TANGENT ".w);"
        "TBN = mat3(T, B, N);"
...
#       endif
    ...
    "{"
        ...
        "vec4 albedoAlpha = maps[ALBEDO].color*fragColor;"
        "if (maps[ALBEDO].active != 0)"
            "albedoAlpha *= TEX(maps[ALBEDO].texture, uv);"
        ...
        "else"
        "{"
            // Add this else branch to make ambient lighting respond to normals 
            // even without an irradiance map
            "float hemisphericFactor = 0.5 + 0.5 * N.y;"  // Simple up-based hemisphere lighting
            "vec3 skyColor = " RLG_SHADER_UNIFORM_COLOR_AMBIENT " * 1.2;"
            "vec3 groundColor = " RLG_SHADER_UNIFORM_COLOR_AMBIENT " * 0.3;"
            "ambient = mix(groundColor, skyColor, hemisphericFactor);"
        "}"
        ...
        GLSL_FINAL_COLOR("vec4(diffuse + specLighting + emission, alpha)")
        //GLSL_FINAL_COLOR("vec4(N.xyz, alpha)")
        //GLSL_FINAL_COLOR("vec4(TBN[0], 1.0)")
    "}"
};
...
void RLG_DrawMesh(Mesh mesh, Material material, Matrix transform)
{
    ...
    int didVaoWork = rlEnableVertexArray(mesh.vaoId);
...
RLG_Skybox RLG_LoadSkybox(const char* skyboxFileName)
{
    ...
    // Generate Irradiance Cubemap
    {
        ...
        skybox.irradiance.id = rlLoadTextureCubemap(NULL, size, skybox.cubemap.format, 1);
...
RLG_Skybox RLG_LoadSkyboxHDR(const char* skyboxFileName, int size, int format)
{
    ...
    // Generate the cubemap for the skybox
    {
        ...
        // Create a cubemap texture to hold the HDR data
        skybox.cubemap.id = rlLoadTextureCubemap(NULL, size, format, 1);
    ...
    // Generate the irradiance cubemap
    {
        ...
        skybox.irradiance.id = rlLoadTextureCubemap(NULL, irrSize, skybox.cubemap.format, 1);
        rlCubemapParameters(skybox.irradiance.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_LINEAR);
        rlCubemapParameters(skybox.irradiance.id, GL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_LINEAR);
